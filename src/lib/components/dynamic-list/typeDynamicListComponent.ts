 ...
 *******************************************************************************/
import {
    RenderingContext, RenderingContextBinding, luceneEscapeKeyValue, Query
} from '@ibm-wch-sdk/ng';
import { queryToString } from '@ibm-wch-sdk/utils';
    ...
    ngOnInit() {
        ...
        this.rcSub = this.onRenderingContext
			.subscribe((rc) => {
                ...
                const fq = [
                    luceneEscapeKeyValue('classification', 'content'),
                    luceneEscapeKeyValue('isManaged', 'true')
                ];
                // filter by site, if type is Design page
                if (type === 'Design page' && rc.context && rc.context.site) {
                    fq.push("{!join from=contentId to=id}siteId:" + rc.context.site.id);
                }
                if (type) {
                  const query: Query = {
                    'q': luceneEscapeKeyValue('type', type),
                    'fq': fq,
                    'sort': 'lastModified desc',
                    'fl': 'document:[json],lastModified',
                    'rows': ROWS,
                    'spaFilter': this.hash
                  };
                  this.queryString = queryToString(query);
                } else {
